...
const Navbar = () => {
  ...
  return (
    <>
      <header className='flex justify-between sticky top-0 p-4 bg-sky-pink-gradient shadow-sm items-center'>
        ...
        <ul className='hidden md:flex gap-4 uppercase font-medium'>
          {authState.isLoggedIn ? (
            <>
              <li className="bg-dark-pink text-white hover:bg-dark-light-pink font-medium rounded-md">
                <Link to='/tasks/add' className='block w-full h-full px-4 py-2'> <i className="fa-solid fa-plus"></i> Add task </Link>
              </li>
              <li className='py-2 px-3 cursor-pointer hover:bg-dark-teal transition rounded-sm' onClick={handleLogoutClick}>Logout</li>
            </>
          ) : (
